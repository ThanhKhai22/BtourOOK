...
const CreatePost = ({ isEdit }) => {
  ...
  const { prices, categories, provinces } = useSelector((state) => state.app);
  ...
  const handleSubmit = async () => {
    ...
    if (result !== 0) {
      if (dataEdit && isEdit) {
        ...
        if (response?.data.err === 0) {
          Swal.fire("Thành công", "Đã thêm thành công", "success").then(() => {
            resetPayload();
          });
        } else {
          Swal.fire("Oops!", "Có lỗi xảy ra", "error");
      ...
      <div className="flex gap-4">
        <div className="py-4 flex flex-col gap-8 flex-auto">
          ...
          <div className="w-full mb-6">
            ...
            <small>Cập nhật hình ảnh rõ ràng sẽ thực hiện nhanh hơn</small>
